...
Module Goal.
  ...
  Theorem goal : forall (d : Z), 0 <= d -> (forall (lo : Z), forall (hi : Z), lo < hi -> (forall (v : tree Z), (d > 0 /\ (~leaf v -> lower_bound v lo) /\ (~leaf v -> upper_bound v hi) /\ bst v /\ ~leaf v /\ (exists (n2 : Z), depth v n2 /\ n2 <= d)) -> (exists (lt : tree Z), exists (rt : tree Z), exists (d_2 : Z), exists (x : Z), d > 0 /\ (lo + 1) < hi /\ lo < x /\ x < hi /\ 0 <= (d - 1) /\ (d - 1) >= 0 /\ (d - 1) < d /\ (~leaf lt -> lower_bound lt lo) /\ (~leaf lt -> upper_bound lt x) /\ bst lt /\ (exists (n : Z), depth lt n /\ n <= (d - 1)) /\ 0 <= d_2 /\ d_2 >= 0 /\ d_2 < d /\ d_2 = (d - 1) /\ (~leaf rt -> lower_bound rt x) /\ (~leaf rt -> upper_bound rt hi) /\ bst rt /\ (exists (n1 : Z), depth rt n1 /\ n1 <= d_2) /\ root v x /\ lch v lt /\ rch v rt /\ (exists (nl : Z), exists (nr : Z), depth lt nl /\ depth rt nr /\ (nl > nr -> depth v (nl + 1)) /\ (nr >= nl -> depth v (nr + 1)))))).
  Proof.
    intros d Hd lo hi Hlh v [Hd1 [Hlb [Hub [Hbst [Hnl [n2 [Hdpt Hn2]]]]]]].
    ...
    destruct (tree_depth_exists l) as [nl Hnl1].
    destruct (tree_depth_exists r) as [nr Hnr1].
    ...
    intuition.
    exists l. exists r. exists (d - 1). exists x.
    intuition.
    - apply (tree_lower_bound_destruct v). intuition.
    - apply (tree_upper_bound_destruct_2 v). intuition.
    - apply (tree_bst_lch_bst v). intuition.
    - exists nl. intuition.
      destruct (nl >? nr) eqn:Hge.
      + assert (nl > nr) by lia. intuition.
        replace n2 with (nl + 1) in Hn2 by (apply (tree_depth_unique v); intuition).
        lia.
      + assert (nr >= nl) by lia. intuition.
        replace n2 with (nr + 1) in Hn2 by (apply (tree_depth_unique v); intuition).
        lia.
    - apply (tree_lower_bound_destruct_2 v). intuition.
    - apply (tree_upper_bound_destruct v). intuition.
